...
def get_dates(**context) -> tuple[str, str]: # ** - бери сколько угодно аргументов и упакуй их в dict. context - содержит среду выполнения задачи. Рекомендуется его использовать ради идемпотентности. Что это такое?  Это свойство оркестрации при многократном повторении давать тот же результат, что и при первом применении, не меняя состояние системы
